...
public class TagsEditText extends AutoCompleteTextView {
    private final static String TAG = TagsEditText.class.getSimpleName();
    ...
    public void addTag(String name) {
        Tag tag = new Tag();
        tag.setIndex(mTags.size());

        int position = 0;
        if (mTags.size() > 0) {
            Tag lastTag = mTags.get(mTags.size() - 1);
            position = lastTag.getPosition() + lastTag.getSource().length() + 1;
        }

        tag.setSource(mIsSpacesAllowedInTags ? name.trim() : name.replaceAll(" ", ""));
        tag.setPosition(position);
        tag.setSpan(true);
        mTags.add(tag);
    ...
    private void removeTagSpan(Editable editable, TagSpan span, boolean includeSpace) {
        ...
        mListener.onRemoveTag(tagIndex);
    ...
    public interface TagsEditListener {
        ...
        void onAddTag(int position);

        void onRemoveTag(int position);
    }
    ...
    public static class TagsEditListenerAdapter implements TagsEditListener {
        ...
        @Override
        public void onAddTag(int position) {
