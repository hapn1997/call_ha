...
public class FloatingViewManager implements ScreenChangedListener, View.OnTouchListener, TrashViewListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) mFloatingViewListener.onTouchStarted();
    ...
    public void addViewToWindow(View view, Options options) {
        ...
        if (isFirstAttach) {
            ...
            if (mTrashView.isTrashEnabled()) mWindowManager.removeViewImmediate(mTrashView);
        }
        // 必ずトップに来て欲しいので毎回貼り付け
        if (mTrashView.isTrashEnabled()) mWindowManager.addView(mTrashView, mTrashView.getWindowLayoutParams());
    }
